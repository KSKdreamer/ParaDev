# 2026-06-14 13:29 Frontend API REST Dynamic Query Row

## Focus

- Continue modularizing the SDK-owned frontend API reference renderer.
- Keep the REST dynamic-query field API table stable while separating row assembly from cell assembly.

## Changes

- Added `_frontend_api_rest_dynamic_query_field_row`.
- Updated `_frontend_api_rest_dynamic_query_field_index_rows` to delegate per-field table-row rendering to the helper.
- Preserved REST operation order, path-parameter filtering, body-field filtering, static query defaults, cell formatting, and rendered markdown output.
