# 2026-06-14 12:01 - Frontend API Input Default Cells
...
- Added `_frontend_api_input_default_cells(...)` in `src/paradev/sdk/frontend_api.py`.
- Routed input-default index row generation through the helper so identity, default value, target, alias, and required-state cells are built in one named row builder.
- Preserved the existing default-field filter, input ordering, and table column order.
