# API Catalog Surface Validation

## Done

- Added API catalog source-row validation for `surfaces` metadata.
- Guarded against empty surface lists, empty surface values, duplicate per-row surfaces, string pseudo-lists, and non-string surface values.
- Added focused architecture tests so the aggregate API catalog cannot silently corrupt `surface_index` grouping.
