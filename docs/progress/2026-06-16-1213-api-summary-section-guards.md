# 2026-06-16 12:13 API summary section guards

## Scope

- Hardened `api_summary_section(...)` so scalar summary line inputs fail with the same contextual list-like error as the rest of the API Markdown helpers.
- Hardened `api_summary_reference_sections(...)` so scalar section collections and scalar section line values fail before they can render as character-split Markdown.
- Added focused regression coverage beside the existing API summary renderer tests.
