# 2026-06-16 12:23 API index row guards

## Scope

- Hardened `index_rows(...)` so scalar index mappings fail with a contextual mapping error instead of falling through to `.items()`.
- Added direct `index_rows(...)` coverage for normal rendering and scalar index inputs.
