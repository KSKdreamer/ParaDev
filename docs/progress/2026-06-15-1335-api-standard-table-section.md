# 2026-06-15 13:35 API standard table section

## Scope

- Added `api_standard_table_section(...)` as the shared renderer for the standard API symbol table section.
- Routed 16 facade API Markdown renderers through the helper instead of repeating the API-standard table heading, header, and row-renderer block.
- Kept code-cell value rendering as the default and preserved plain Markdown value rendering for desktop, games, templates, and copy-root tables through `markdown_value=True`.
- Added focused helper assertions for both row-rendering modes.
...
- Stage only the shared Markdown helper, facade renderer table-section cleanup, helper tests, and this note.
