# 2026-06-15 13:22 API regeneration command block

## Scope

- Added `api_regeneration_command_block(...)` as the shared fenced Bash block renderer for generated API reference regeneration commands.
- Routed 16 facade API Markdown renderers through the helper instead of repeating raw fenced-command lines.
- Kept each facade title, source marker, regenerate-when sentence, overview sections, API-standard table heading, and row renderer unchanged.
- Added a focused helper test that pins the exact fenced Bash command output.
...
- Stage only the shared Markdown helper, facade renderer command-block cleanup, helper tests, and this note.
