# API Markdown Table Object Validation

## Done

- Added contextual validation that generated API Markdown table payloads must be mappings before table-field lookup.
- Added validation that API summary module labels must be strings.
- Routed generated API reference section helpers through the shared table guard so scalar payloads fail with API-specific errors.
- Added regression coverage for scalar table payloads and non-string module labels.
