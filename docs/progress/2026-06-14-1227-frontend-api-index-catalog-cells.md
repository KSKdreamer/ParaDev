# 2026-06-14 12:27 - Frontend API Index Catalog Cells

## Focus

- Continued the frontend API reference renderer cleanup around the generated index catalog table.
...
- Added `_frontend_api_index_catalog_cells(...)` in `src/paradev/sdk/frontend_api.py`.
- Routed index catalog row generation through the helper so catalog id, raw contract path, Python helper, TypeScript helper, and usage cells are built in one named row builder.
- Preserved the existing catalog ordering and table column order.
