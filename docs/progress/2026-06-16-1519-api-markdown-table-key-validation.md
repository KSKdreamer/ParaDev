# API Markdown Table Key Validation Progress

Date: 2026-06-16 15:19

Linear: N/A

## Done

- Added table-level non-string key validation to the shared API Markdown table guard.
- Covered `api_index_sections` and `api_summary_lines` so invalid API table keys fail with a direct `TypeError` instead of leaking into missing-field behavior or silently rendering.

## Verification

- `rtk bash scripts/test.bash tests/test_api_table.py::test_api_index_sections_rejects_non_string_table_keys tests/test_api_table.py::test_api_summary_lines_rejects_non_string_table_keys -q`
...
- Full suite deferred to keep CPU free while other PIHC3 migration workers are active.
- Unrelated dirty skill, desktop, PIHC3, loader, and local asset changes were left untouched.

## Next

- Continue tightening API Markdown table validation and generated-reference contracts in small, reviewable slices.
