# 2026-06-15 13:10 API standard index sections

## Scope

- Added `api_standard_index_sections(...)` as the shared renderer for the standard Module, Feature, and Kind API index sections.
- Routed 16 facade API Markdown renderers through the helper instead of repeating the same index-section block.
- Kept each facade renderer's summary section, API-standard table section, and row renderer unchanged.
- Added a focused helper test that pins section order, headers, row counts, and blank-line spacing.
