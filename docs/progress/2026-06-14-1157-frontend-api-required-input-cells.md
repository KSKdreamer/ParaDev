# 2026-06-14 11:57 - Frontend API Required Input Cells
...
- Added `_frontend_api_required_input_cells(...)` in `src/paradev/sdk/frontend_api.py`.
- Routed required-input index row generation through the helper so identity, target, alias, and option-source provider cells are built in one named row builder.
- Preserved the existing required-field filter, input ordering, and table column order.
