# 2026-06-16 16:42 - API table payload helper
...
- Preserved public SDK, CLI, REST, MCP, and generated API table payload behavior.

## Changes

- Added `_api_table_payload` as the single private initializer for generated table `schema` and `row_count` fields.
- Routed `api_standard_table` and `api_indexed_table` through the shared payload initializer.
- Kept table key ordering, index generation, row copying, validation messages, and public helper signatures unchanged.
