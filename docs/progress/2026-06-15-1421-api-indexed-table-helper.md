# 2026-06-15 14:21 API indexed table helper

## Scope

- Added `api_indexed_table(...)` as the shared private payload builder for custom API tables with named indexes.
- Routed CLI, MCP, Project object, and API catalog table payload assembly through the helper while preserving schema, row count, index order, and detached row copies.
- Kept list-field index expansion and empty-index skipping explicit at each caller so the table contracts remain self-documenting.
- Added a focused helper test for custom value fields, named index order, list expansion, skipped empty index values, and row list detachment.
...
- `rtk uv run black src/paradev/_api_table.py src/paradev/surfaces/cli.py src/paradev/surfaces/mcp.py src/paradev/sdk/project_api.py src/paradev/surfaces/api_catalog.py tests/test_api_table.py`: unchanged.
...
- Stage only the indexed-table helper, CLI/MCP/Project/API catalog routing, helper tests, and this note.
