# Frontend API MCP Tool Input Index Progress

Date: 2026-06-14 04:50 +0800

Linear: none

## Done

- Added generated MCP Tool Input Index tables to the frontend API reference renderer.
- The new table lists MCP-bound operation inputs with tool name, operation id, field, type, required/default state, normalizer target bucket, and adapter-side alias.
- Regenerated `docs/user-manual/frontend-api-reference.md`.
- Updated the frontend API guide to mention the MCP input audit table.
- Added targeted architecture and CLI assertions for project creation, project inspection, and PDX parse MCP input rows.
...
- `rtk git diff --check -- src/paradev/sdk/frontend_api.py tests/test_architecture.py tests/test_cli.py docs/user-manual/frontend-api.md docs/user-manual/frontend-api-reference.md docs/progress/2026-06-14-0450-frontend-api-mcp-tool-input-index.md`
