# 2026-06-15 14:54 API reference document helper

## Scope

- Added `api_reference_markdown(...)` as the shared Markdown document renderer for generated API reference pages.
- Routed `api_standard_reference_markdown(...)` through the new helper so standard references share the same title, intro, and section-separator boundary.
- Routed Project object, CLI, MCP, and API catalog custom reference renderers through the helper while preserving their sections and generated output.
- Added focused helper coverage for title, generated intro, regeneration command block, and section blank-line layout.
...
- Stage only the reference-document helper, renderer rollout, helper test, and this note.
