# 2026-06-14 13:22 Frontend API REST Request Planner Row

## Focus

- Continue modularizing the SDK-owned frontend API reference renderer.
- Keep the REST request planner API table stable while isolating row assembly from cell assembly.

## Changes

- Added `_frontend_api_rest_request_planner_row`.
- Updated `_frontend_api_rest_request_planner_index_rows` to delegate per-operation table-row rendering to the helper.
- Preserved REST operation order, method/path/query/path-parameter/body-field cells, operation-id cells, and rendered markdown output.
