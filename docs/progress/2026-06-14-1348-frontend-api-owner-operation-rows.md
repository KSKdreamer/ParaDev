# Frontend API Owner Operation Rows Progress

Date: 2026-06-14 13:48
...
- Added per-owner summary and detail row helpers for owner operation API tables.
- Preserved owner order, owner id lookup, operation grouping, callback cell assembly, and markdown output.
...
- Continue extracting focused row-level helpers in `src/paradev/sdk/frontend_api.py` where the generated API table shape stays unchanged.
