# API Catalog Helper Validation

## Done

- Validated both `table_helper` and `markdown_helper` metadata when API catalog source rows are materialized.
- Reused the same validation path for the `api-catalog` row instead of bypassing source metadata.
- Added focused architecture tests for callable helper checks and contextual missing/non-callable helper failures.
...
- Continue hardening generated API-reference contracts while preserving existing SDK, CLI, REST, MCP, and GUI-facing behavior.
