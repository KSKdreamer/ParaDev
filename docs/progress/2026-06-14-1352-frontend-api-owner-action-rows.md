# Frontend API Owner Action Rows Progress

Date: 2026-06-14 13:52
...
- Added a per-owner action summary row helper for owner action API tables.
- Preserved owner order, owner id lookup, operation/action grouping, callback cell assembly, and markdown output.
...
- Continue extracting focused table-row helpers in `src/paradev/sdk/frontend_api.py` where the public API reference output stays unchanged.
