# Frontend API Detail Row Helper Rollout

## Scope

- Continued the behavior-preserving frontend API renderer cleanup stream.
- Routed the remaining direct Markdown row builders through `_frontend_api_table_row`.
- Covered REST input detail rows, binding input/index rows, option-source/provider rows, and form/input detail indexes.
- Left public frontend API behavior and generated Markdown format unchanged.
