# Frontend API Options Resolved Payload

Date: 2026-06-14 18:36 Asia/Shanghai

## Summary

- Added `_frontend_api_options_resolved_payload` for provider-payload response population.
- Routed `resolve_frontend_api_options` through the helper after provider execution.
- Kept generated frontend API Markdown and TypeScript output unchanged.

## Verification

- Runtime probe for provider payload schema, option rows, count, and base-payload mutation behavior.
