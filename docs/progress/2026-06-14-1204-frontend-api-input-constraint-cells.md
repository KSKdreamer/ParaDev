# 2026-06-14 12:04 - Frontend API Input Constraint Cells
...
- Added `_frontend_api_input_constraint_cells(...)` in `src/paradev/sdk/frontend_api.py`.
- Routed input-constraint index row generation through the helper so identity, choices, minimum, and required-state cells are built in one named row builder.
- Preserved the existing choices/minimum filter, input ordering, and table column order.
