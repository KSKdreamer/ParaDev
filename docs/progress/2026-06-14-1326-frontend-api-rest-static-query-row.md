# 2026-06-14 13:26 Frontend API REST Static Query Row

## Focus

- Continue modularizing the SDK-owned frontend API reference renderer.
- Keep the REST static-query API table stable while separating row assembly from cell assembly.

## Changes

- Added `_frontend_api_rest_static_query_row`.
- Updated `_frontend_api_rest_static_query_index_rows` to delegate per-query table-row rendering to the helper.
- Preserved REST operation order, static-query key sorting, query filtering, cell formatting, and rendered markdown output.
